...
def main() -> None:
    ...
    # Use GPU if available; otherwise use float16 on CPU to cut memory ~in half
    if torch.cuda.is_available():
        ...
        dtype = torch.float16
        device_map = "cpu"
        print("Using CPU with float16 (reduced memory, slower math).")

    model = AutoModelForCausalLM.from_pretrained(
        MODEL_PATH,
        dtype=dtype,               # new-style arg (no deprecation warning)
        device_map=device_map,
        low_cpu_mem_usage=True,    # avoid extra copies while loading
    )
    model.eval()
    ...
    with torch.no_grad():
        outputs = model.generate(
            **inputs,
            max_new_tokens=64,  # smaller to keep generation light
            do_sample=False,
